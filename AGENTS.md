...
## This app: Pixel Open, CriblCon 26 Edition

A port of the Next.js game in `../tennis-pixel-game`, dressed for Cribl. Notes for working on it:

- **No Capra UI.** The game draws its own pixel-art UI with Tailwind v4 (wired through `.postcssrc.mjs`), so `main.tsx` does not import Capra's stylesheets: their unlayered base rules would restyle the page. Capra is still used for the Cribl marks: `src/game/art/cribl.ts` reads SVG path data off `@capra/icons` components and rasterizes it into pixel masks. The game keeps its dark palette in both Cribl themes, like video.
- **Saves go to the KV store**, never browser storage: `src/game/state/kvStorage.ts` is the zustand storage adapter. The KV store is per app, so the key is per user: `pixel-open-v1/users/<id>` from `window.getCriblUser()`. Writes are debounced, skipped if the first read failed, and never delete. Outside Cribl there is no `CRIBL_API_URL`, so nothing is saved.
- **Frontend only.** The scaffold's sample backend endpoint was removed, and `config/policies.yml` declares nothing.
- **Goat mode** (`settings.goats`, on by default): `src/game/art/goat.ts` turns a `PlayerLook` into a goat; the renderer, crowd and audio (bleats) follow it.
- **Venues** are data in `src/game/sim/venues.ts` (boards, tickers, product decals). CriblCon 26 is the tour finale and the default Quick Match venue.
- Tests: `npm test` (Vitest, `vitest.config.ts`, kept apart from `vite.config.ts`). `npm run format` runs Prettier on `src/` only, so the scaffold-managed files keep their shape for `apps upgrade`.
